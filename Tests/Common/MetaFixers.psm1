...
function Get-TextFilesList {
    ...
    $allTextFiles = Get-ChildItem -File -Recurse $root | Where-Object { @('.gitignore', '.gitattributes', '.ps1', '.psm1', '.psd1', '.json', '.xml', '.cmd', '.mof') -contains $_.Extension }
    if ($notMatch -ne '') {
        $allTextFiles | Where-Object 'DirectoryName' -NotMatch $notMatch
    }
    else {
        $allTextFiles
    }
