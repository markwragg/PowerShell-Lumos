...
* `Invoke-Lumos` now also broadcasts `WM_THEMECHANGED` alongside the existing `WM_SETTINGCHANGE`, matching the broadcast Windows' own Settings app (and Microsoft's PowerToys "Light Switch" module) sends. This doesn't change what it fixes in practice - already-open File Explorer windows still don't repaint without `-RestartExplorer`, which appears to be a genuine Windows 11 limitation rather than something missing here (PowerToys has the [same open issue](https://github.com/microsoft/PowerToys/issues/42463)) - but it's one step closer to matching what the OS itself does.
